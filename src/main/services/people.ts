...
async function loadLifeYears(personIds: string[]): Promise<Map<string, { birthYear?: number | null; deathYear?: number | null }>> {
  ...
  for (const ev of rows) {
    ...
    if (ev.type === 'birth') entry.birthYear = ev.dateYear
    if (ev.type === 'death') entry.deathYear = ev.dateYear
...
export async function updatePerson(input: UpdatePersonInput): Promise<PersonDetail> {
  ...
  if (input.death === null) {
    if (detail.deathEvent?.id) await deleteEvent(detail.deathEvent.id)
...
export async function searchPlaces(query: string): Promise<Array<{ id: string; name: string }>> {
  ...
  const rows = await db
    ...
    .where(
      trimmed
        ? and(isNull(schema.places.deletedAt), like(schema.places.normalizedName, `%${trimmed}%`))
