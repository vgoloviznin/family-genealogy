...
import type { CreatePersonInput, FamilySummary, PedigreeType, PersonDetail, UnionType } from '@shared/types';
...
async function ensureFamilyActive(familyId: string): Promise<void> {
  const db = getDatabase();
  const ts = nowIso();
  await db.update(schema.families).set({ deletedAt: null, updatedAt: ts }).where(eq(schema.families.id, familyId));
}

async function cleanupEmptyFamily(familyId: string): Promise<void> {
  const db = getDatabase();
  const [family] = await db
    .select()
    .from(schema.families)
    .where(and(eq(schema.families.id, familyId), isNull(schema.families.deletedAt)));
  if (!family) {
    return;
  }

  const partners = await countActiveFamilyPartners(familyId);
  const children = await countActiveFamilyChildren(familyId);
  if (partners === 0 && children === 0) {
    const ts = nowIso();
