...
export async function getTree(personId?: string | null): Promise<TreeData> {
  const db = getDatabase();

  const peopleRows = await db.select().from(schema.people).where(isNull(schema.people.deletedAt));

  if (peopleRows.length === 0) {
    return { nodes: [], edges: [], families: [], focusPersonId: null };
  }

  const families = await db.select().from(schema.families).where(isNull(schema.families.deletedAt));
