...
import { createTestProjectDir, createTestProjectFiles } from '../test/project-fixture'
import { isSqliteAvailable } from '../test/sqlite-available'
import { closeProject, openProjectAtPath, updateProjectName, listRecentProjects } from './project'
...
vi.mock('./settings', () => ({
  ...
  pruneRecentProjects: (missing: string[]) => {
    ...
    for (let i = recentPaths.length - 1; i >= 0; i--) {
      if (drop.has(recentPaths[i])) recentPaths.splice(i, 1)
    }
  },
