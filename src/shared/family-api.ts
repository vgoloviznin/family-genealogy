...
export function createFamilyApi(invoke: Invoke): Api['family'] {
  return {
    ...
    dissolveUnion: (familyId, personId) =>
      invoke(IPC_CHANNELS.FAMILY_DISSOLVE_UNION, familyId, personId) as ReturnType<Api['family']['dissolveUnion']>,
...
export function assertFamilyApi(family: Partial<Api['family']> | undefined): void {
  if (!family) {
    throw new Error('window.api.family is not defined');
  }
