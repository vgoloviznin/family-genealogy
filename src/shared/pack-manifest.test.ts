...
import { validatePackManifest, verifyPackDatabaseHash } from './pack-manifest'
import { createEmptyDir, createTestProjectDir, removeDir } from '../main/test/project-fixture'
import { isSqliteAvailable } from '../main/test/sqlite-available'
...
describe('pack manifest', () => {
  ...
  it('detects sqlite hash mismatch', async () => {
    ...
    try {
      await expect(
        verifyPackDatabaseHash({ format: 'fgtree', sqliteSha256: 'deadbeef' } as never, dbPath, sha256File)
      ).rejects.toThrow('Кontрольная сумма базы данных не совпадает')
    } finally {
      removeDir(dir)
    }
  })
